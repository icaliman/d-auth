...
module.exports = (options) ->
  ...
  expressApp.use (req, res, next) ->
    ...
    if req.isAuthenticated()
      ...
      $user = model.at "auths.#{req.session.userId}"
      $user.fetch (err) ->
        if opts.passUser
          user = $user.get()
          # Delete salt and hashed_password so they will not be disposed on the client side
          delete user.local.salt
          delete user.local.hashed_password
          model.set "_session.user", user
        $user.set("timestamps.loggedin", +new Date, next)
    else next()

  # Setup static passport authentication routes
  setupStaticRoutes expressApp, opts.strategies
...
setupPassport = (strategies) ->
  ...
  passport.use new LocalStrategy opts.passport, (req, username, password, done) ->
    ...
    $uname.fetch (err) ->
      ...
      if hashed is auth.local.hashed_password
